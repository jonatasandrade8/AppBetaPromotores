 ...
 * (MODIFICADO: Implementada a lógica de travamento do Promotor autenticado)
 */
function loadAndPopulateDropdowns() {
    const APP_DATA = QdeliciaData.getAppData();
    const RELATORIO_DATA = QdeliciaData.RELATORIO_DATA;
    ...
    if (authenticatedPromotor) {
        if (selectPromotor) {
            ...
            selectPromotor.style.backgroundColor = '#e9ecef'; 
            populateRede(authenticatedPromotor);
        }
        
        if (savedSelection && savedSelection.rede) {
            selectRede.value = savedSelection.rede;
            populateLoja(authenticatedPromotor, savedSelection.rede);
            if (savedSelection.loja) {
                selectLoja.value = savedSelection.loja;
            }
        }
    } else if (savedSelection && savedSelection.promotor) {
...
// O Listener de Promotor (abaixo) precisa ser mantido para permitir que a seleção de Rede/Loja seja salva
// mesmo que o campo esteja desabilitado.
if (selectPromotor) {
