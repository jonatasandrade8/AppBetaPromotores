 ...
 * *** IMPLEMENTAÇÃO DA AUTENTICIDADE/TRAVAMENTO DO PROMOTOR ***
 */
function loadAndPopulateDropdowns() {
    const PHOTO_TYPES = QdeliciaData.PHOTO_TYPES;
    const APP_DATA = QdeliciaData.getAppData();
    ...
    // 3. >>> IMPLEMENTAÇÃO DA AUTENTICIDADE: Obter o Promotor Logado e Preencher/Travar <<<
    // CORREÇÃO: Usamos 'promotorNome' em vez de 'promotorId' para bater com as chaves do APP_DATA
    const authenticatedPromotor = localStorage.getItem('promotorNome'); 
    
    if (authenticatedPromotor) {
        // A. Seleciona o promotor logado
        selectPromotor.value = authenticatedPromotor; 
        
        // B. Bloqueia o dropdown para garantir autenticidade
        selectPromotor.disabled = true;
        selectPromotor.style.backgroundColor = '#e9ecef'; // Visual cinza de bloqueado
        
        // C. Garante que a Rede seja carregada com base no promotor logado
        populateRede(authenticatedPromotor);

        // D. Se houver uma Rede/Loja salva no localStorage, restaura a seleção APENAS se for do mesmo promotor
        if (savedSelection && savedSelection.promotor === authenticatedPromotor) {
            if (savedSelection.rede) {
                selectRede.value = savedSelection.rede;
                populateLoja(authenticatedPromotor, savedSelection.rede); // Usa o promotor autenticado
        ...
        // Fallback: Se não houver promotor logado (teste ou erro), libera o campo
        selectPromotor.disabled = false;
        selectPromotor.style.backgroundColor = ''; 
...
if (selectPromotor) {
    // O listener DEVE ser mantido, mas sua execução será bloqueada pelo `disabled=true` do campo
